...
const ModifyPw: React.FC<ModifyPwProps> = () => {
  ...
  const handlePwUpdate = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    try {
      toast.loading("Sto cambiando la tua password", {
        position: "top-center",
        toastId: "loading",
        theme: "dark",
      });
  ...
  return (
    <main>
      <p className="text-4xl text-center text-white dark:text-white font-medium uppercase mt-10 mb-5">
        {" "}
        Modifica Password{" "}
      </p>
      <form className="space-y-6 px-6 pb-4" onSubmit={handlePwUpdate}>
        ...
        <button
          type="submit"
          value="submit"
          className="w-full text-white focus:ring-blue-300 font-medium rounder-lg text-sm px5 py-2.5 text-center bg-brand-orange hover:bg-brand-orange-s rounded"
        >
          {updating ? "Modificando..." : "Modifca Password"}
        </button>
      </form>
    </main>
  );
