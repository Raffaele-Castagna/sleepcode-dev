      ...
      <tbody className="text-white">
        {problems.map((problem, idx) => {
          ...
          return (
            ...
            >
              <th>
                <div className="px-2 py-2 font-medium whitespace-nowrap text-dark-green-s">
                  {" "}
                  {solvedProblems.includes(problem.id) && (
                    <BsCheckCircle fontSize={"15"} width="18" />
                  )}{" "}
                </div>
                <div className="px-2 py-2 font-medium whitespace-nowrap text-dark-pink">
                  {" "}
                  {likedProblems.includes(problem.id) && (
                    <BsHeartFill fontSize={"15"} width="18" />
                  )}
                </div>
              </th>
              ...
              <td className={`px-6 py-4 ${diffcolor}`}>{problem.difficulty}</td>
                ...
                {problem.videoId ? (
                  <AiFillYoutube
                    ...
                    onClick={() =>
                      setYoutubePlay({
                        isOpen: true,
                        video: problem.videoId as string,
                      })
                    }
                  />
                ) : (
